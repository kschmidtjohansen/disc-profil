    ...
    return (
      <div className="min-h-screen bg-background">
        <header className="bg-primary text-primary-foreground px-6 py-4 flex justify-between items-center">
          <h1 className="text-xl font-semibold" style={{ fontFamily: 'Playfair Display, serif' }}>DiSC Profilering</h1>
          ...
          <Card>
            ...
            <CardContent className="space-y-6">
              <RadioGroup value={answers[currentQuestion] || ""} onValueChange={handleAnswer}>
                {q.options.map((opt) => (
                  <div key={opt.style} className="flex items-center space-x-3 p-3 rounded-lg border hover:bg-muted transition-colors cursor-pointer">
          ...
          <Card className="border-0 shadow-lg">
            <CardHeader className="text-center space-y-4">
              <div className="mx-auto w-20 h-20 rounded-full bg-accent flex items-center justify-center">
                <span className="text-accent-foreground text-3xl font-bold">{discResult}</span>
              </div>
      ...
      <main className="max-w-2xl mx-auto p-6 mt-16">
        <Card className="text-center border-0 shadow-lg">
          ...
          <CardContent className="pt-6">
            <Button size="lg" onClick={() => setTestStarted(true)} className="text-base px-8">
